...
// Fetch and display Castaic Reservoir Outflow data from CDEC
async function fetchCastaicOutflowByRange(startDate, endDate) {
    const apiUrl = `https://cdec.water.ca.gov/dynamicapp/req/JSONDataServlet?Stations=CAS&SensorNums=23&dur_code=H&Start=${startDate}&End=${endDate}`;
    ...
    try {
        ...
        if (Array.isArray(data) && data.length > 0) {
            ...
            // Select up to 10 evenly spread points within the range
            let sampled = [];
        ...
        loadingMessage.style.display = 'none';
        dataContainer.innerHTML = `<p style="color: red;">Failed to load outflow data: ${error.message}</p>`;
        if (castaicOutflowChart instanceof Chart) {
            castaicOutflowChart.destroy();
            castaicOutflowChart = null;
        }
